use crate::Raz_vm::builtins::get_builtin;
use crate::Raz_vm::error::{ErrorType, RazError};
use crate::Raz_vm::instructions::Instruction;
use crate::Raz_vm::value::{ApplyOperatorResult, CallResult, GetPropertyResult, Value};
        ...
        while index < l {
            ...
            match i {
                ...
                Instruction::Call(x) => {
                    ...
                    if let Value::RazFunction(idx, name, params) = callee {
                        if *x != params.len() {
                            self.error = Some(RazError::ArgumentError(format!(
                ...
                Instruction::MakeCode(x, y, z, l) => {
                    self.push_free(Value::RazFunction(
                        *x + padding as usize,
                        y.clone(),
                        z.clone(),
                    ));
