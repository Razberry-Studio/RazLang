...
use crate::Raz_vm::value::{CallResult, GetPropertyResult, NModule, Value, FT};
...
fn push_vector_fn_internal(_this: &Value, arguments: Vec<usize>, heap: &mut Heap) -> CallResult {
    if arguments.is_empty() {
        return CallResult::Error(RazError::ArgumentError(format!(
    ...
    CallResult::Error(RazError::ArgumentError(format!(
        "Expected 1st argument to be reference to vector, got type {}",
        first.value_type().to_string()
    )))
}
